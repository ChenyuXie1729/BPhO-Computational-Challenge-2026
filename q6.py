...
def ring_radius_mm(V, d):
    ...
    return 2 * r_tube * np.sin(theta)

fig = plt.figure(figsize=(12, 6))
fig.suptitle("Electron diffraction",
...
ax_verify.set_ylabel(r'$\sin\theta$')
...
for label, d in d_values.items():
    ...
    theta = bragg_angle(lam, d)
    ax_verify.plot(1/np.sqrt(V_range), np.sin(theta), '-', color=colors[label],
...
def update(_):
    ...
    for label, d in d_values.items():
        theta = bragg_angle(lam, d)
        R = 2 * r_tube * np.sin(theta)
        rings[label].set_radius(R)
        markers[label].set_data([1/np.sqrt(V)], [np.sin(theta)])
